...
import { Observable, from as ObservableFrom, throwError as ObservableThrowError } from 'rxjs';
...
export class LoadableService {
  ...
  constructor(
    @Inject(LOADABLE_MANIFESTS) private manifests: Array<LoadableManifest>,
  ...
  getComponentFactory<T>(componentId: string, injector?: Injector): Observable<ComponentFactory<T>> {
    ...
    const componentPromise: Promise<ComponentFactory<any>> = this.loader.load(manifest.loadChildren)
